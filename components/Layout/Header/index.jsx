    ...
    return (
        <header className="border-b">
            <nav className="navbar container max-w-[50rem] mx-auto px-4 flex filter bg-white h-[56px] items-center justify-between">
                ...
                <Logo />
                <ul className={`mobile-menu md:hidden z-50 overflow-hidden absolute top-[57px] p-4 left-0 right-0 bg-white transition-transform ease-[cubic-bezier(1, 0.5, 1, 0.25))] ${open ? "h-auto translate-y-0 visible border-b" : "h-0 translate-y-1 invisible"}`}>
                    {open &&
                        NavData.map((navdata, index) => {
                            return (
                                <MobileMenu key={index} {...navdata} />
                            )
                        })
                    }
                </ul>
